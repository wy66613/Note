...
有一个n级台阶，每级台阶上都有若干个石子，其中第i级台阶上有ai个石子（i≥1）。两位玩家轮流操作，每次操作可以从任意一级台阶上拿若干个石子放到下一级台阶中（不能不拿）。已经拿到地面上的石子不能再拿，最后无法进行操作的人视为失败。问如果两人都采用最优策略，先手是否必胜。
...
给定n堆石子以及一个由k个不同正整数构成的数字集合S。现在有两名玩家轮流操作，每次操作可以从任意一堆石子中拿去石子，每次拿取的石子数量必须包含于集合S，最后无法进行操作的人视为失败。问如果两人都采用最优策略，先手是否必胜。
...
{
	cin>>m;
	for(int i=0;i<m;i++) cin>>s[i]; // 读入k个可拿取的石子个数
...
设S表示一个非负整数集合，定义mex(s)为求出不属于集合S的最小非负整数的运算，即：mex(S)=min(x)，x属于自然数且x不属于S。
