...
内联函数相较于常规函数减少了函数调用这一过程，而是将内联函数的编译代码与其他程序代码内联起来。但是会占用更多的内存。如果调用次数很多，则程序中含有的函数的副本相应的也有很多个。做法是在函数定义前加上关键字**inline**。通常的做法是省略原型，将整个定义放在原型所在位置。内联函数的值传递与常规函数无区别。
